...
func (uc *uniCache) SafeEncode(data []byte, appendIdx uint64, encodedID uint32) ([]byte, []byte) {
    ...
    fmt.Printf("[SafeEncode] index=%d didnt find have data for for ID=%d, capacity=%d, cache size=%d, evicted size=%d, last element=%d \n",
        appendIdx, encodedID, uc.capacity, len(uc.cache), len(uc.evicted), uc.cache[len(uc.cache)])
	return data, nil
}
