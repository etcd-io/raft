...
function validate {
    ...
    set -o pipefail
    env JSON="${trace}" java -XX:+UseParallelGC -cp ${workdir}/tool/tla2tools.jar:${workdir}/tool/CommunityModules-deps.jar tlc2.TLC -config "${config}" "${spec}" -lncheck final -metadir "${workdir}/states" -fpmem 0.8 | tee ${log} | sed -nuE "s/<<\"Progress %:\", ([0-9]+)>>$/${id} \1/p"
